...
function setValue(dictionary, key, value) {
  ...
  if (dictionary.has(key)) {
    ...
    // Якщо ключ не існує, додаємо нову пару ключ-значення до словника, де значення буде таке ж, як і ключ
...
function addKeyValuePairs(dictionary, entries) {
  ...
  entries.forEach(([key, value]) => {
    // Перевіряємо, чи словник вже містить такий ключ за допомогою методу has
    if (!dictionary.has(key)) {
      // Якщо ключ є унікальним, додаємо його до словника за допомогою методу set та збільшуємо added на 1
      dictionary.set(key, value);
      added++;
    } else {
      // Якщо ключ не є унікальним, збільшуємо rejected на 1
      rejected++;
    }
  });

  // Повертаємо об'єкт з dictionary, added та rejected
  return {
    dictionary,
    added,
    rejected,
